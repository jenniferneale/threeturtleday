...
const archive = {
    ...
    ,
    b20Hands: "<img class=\"img-responsive pull-left rounded story-pic\" src=\"/images/DinosaurJFNeale.jpg\"><br>"+"<div class=\"MsoNormal\" style=\"line-height: 200%; text-indent: .5in;\">" +
...
"<div class=\"MsoNormal\" style=\"line-height: 200%; text-indent: .5in;\">" +
"“Mum, who was that?”<span style=\"mso-spacerun: yes;\">&nbsp; </span>Jim entered the kitchen, taking " +
"his jacket from where it hung by the door.</div>" +
"<div class=\"MsoNormal\" style=\"line-height: 200%; text-indent: .5in;\">" +
"“Just a peddler.<span style=\"mso-spacerun: yes;\">&nbsp; </span>Mind you stay away from him- " +
...
"<div class=\"MsoNormal\" style=\"line-height: 200%; text-indent: .5in;\">" +
"Jim nodded and let me give him a kiss on his forehead before setting out.</div>" +
"<div class=\"MsoNormal\" style=\"line-height: 200%; text-indent: .5in;\">" +
"We live in a" +
...
"<div class=\"MsoNormal\" style=\"line-height: 200%; text-indent: .5in;\">" +
"“Maryann!” Mrs. Fox cried as she opened the door to her shop.<span style=\"mso-spacerun: yes;\">&nbsp;" +
...
"“Make a wish,” he said.</div>"
    ...
    b23Kittens: /*"<object type=\"application/x-java-applet\" WIDTH=400 HEIGHT=200>\"><param name=\"code\" value=\"KittenAdventure\"></param><param name=\"archive\" value=\"https://www.dropbox.com/s/9jmmy4rex70mp42/KittenAdventure.jar\" /></object>"+*/
    "<img class=\"img-responsive\" src=\"/images/KittenScreenshot.png\"><br>"+"<p>A short proof-of-concept role-playing game in which you, the adventurer, must deal with a dragon. This Java applet was written in Eclipse using the ACM library. Available <a href=\"https://www.dropbox.com/s/9jmmy4rex70mp42/KittenAdventure.jar\">here</a>.</p>"
    ,

    b24Frank: "<img class=\"img-responsive pull-left rounded story-pic\" src=\"/images/FrankCloseup.jpg\"><br>"+
    ...
    "<p>This game is written in C# in Unity. A semi-playable version is available here: <a href=\"https://www.dropbox.com/s/ndyo13soybelhhi/FrankvTheVampires.zip?dl=0\">Download for PC.</a><p>"+
    ...
    "<p>In this case, our goal was twofold: First, we were permitted to look at the electron microscopes from a respectful distance under the watchful eye of the female researcher in charge of that lab. Aaron explained how the elderly machine he primarily uses had been retrofitted to send images to a computer monitor, instead of to a film plate. Imagine, he asked, the days when we had to walk these over to a dark room and develop hundreds of images ourselves, praying not to ruin priceless shots of cell anatomy, after weeks spent freezing and fixing the specimens, shrinking tiny \"nodding bean\" protists to sturdy raisins. Next, he took us to see the live protists themselves: <em>Breviata</em> and <em>Ancyromonas</em>, swimming in tiny plastic specimen jars under the microscope, tiny dots propelled by flagella, surrounded by the even teenier dots of the bacteria they graze on.</p>" +
    "<img class=\"img-responsive pull-left col-sm-4 rounded story-pic\" src=\"/images/microbe-images-small.gif\"><br>"+
	"<p>\"Where do the specimens come from?\" I asked. \"How do you find them?\"</p>" +
	"<p>\"Oh,\" he said, \"from all over the world. Any time we go to a conference, we collect a little water. Sometimes the most exotic places have nothing of interest; sometimes the most mundane locations have the most exciting finds; sometimes the other way around. One of these we discovered in a two-year-old bottle of seawater from the Baltic that had been sitting forgotten on a shelf. These eukaryotes aren't very common, but they do appear all over the world.\"</p>" +
    "<img class=\"img-responsive pull-right col-sm-4 rounded story-pic\" src=\"/images/3D.png\"><br>"+
    "<p>The trouble was, we learned, that piecing together the results of his electron microscopy is staggeringly time consuming. His current process involves manually tracing each image, importing the curves into 3D visualization software, and manually smoothing the shapes generated into something that actually looks like the structure for anchoring flagella. (The imported 3D tubes look horrifically bent out of shape initially, hence the challenge title \"Iron Out the Kinks\"!)</p>" +
    "<p>This particular hackathon is organized by self-selection, so while some challenges had multiple large teams finding multiple solutions to the same problem, some are tackled by only a few people, as with my team that gathered to take on \"Iron Out the Kinks\" on Saturday. For a difficult task, we had only four people: three developers (<a href=\"https://github.com/abepeterkin\">Abe</a>, <a href=\"https://github.com/heavyimage\">Jesse</a>, and myself) and a designer (Maret). I have some background in machine learning, but the problem seemed particularly difficult to approach in OpenCV or similar as a visual analysis issue. If we humans couldn't identify the cell organs and tubes that required Dr. Aaron's well-trained eye, how could a computer hope to out-do our brains as visual pattern recognition machines? Or, more importantly, how could we begin to tackle such a problem in less than 24 hours? Jesse, who has a background in visual effects for film suggested that we instead focus on creating a streamlined interface for Aaron that had all the advantages of something like Nuke, a VFX editor, with interpolation and automated image stabilization based on manually selected static points.</p>" +
    "<img class=\"img-responsive pull-left col-sm-4 rounded story-pic\" src=\"/images/Image Adjustments (1).png\"><br>"+
	"<p>We settled on creating a lightweight desktop app using Electron.js (since two of us were very familiar with app creation and Javascript, though not Electron itself), and Processing for the meat of the interface, since it has good tools for creating a tailored graphics editor and we had one determined coder who knew it well. Processing-js would be used to import the Processing language .pde files into our Javascript Electron app.</p>" +
    "<img class=\"img-responsive pull-right col-sm-4 rounded story-pic\" src=\"/images/Image Annotations (1).png\"><br>"+
    "<p>We did not make much headway as quickly as we would have liked, but it is hard to complain: the Museum compensated our labor with a hearty dinner in the cafeteria, a breath-taking show about the open source OpenSpace project in the blue glow of the planetarium (complete with the roomful of hackers giggling over a \"Space.exe has stopped unexpectedly\" moment), and a pleasant hour-long reception with wine and networking.</p>" +
	"<p>Everyone worked late into the night. Sometimes code flowed from our fingers; just as often, we were debugging the kind of simple errors that sneak in when your brain is dead at 4:30am. But we kept going. I finally passed out on a cot, wrapped in my coat, at 5am, dozing fitfully under the shadow of the whale. We sleeping hackers lay in neat but shifting rows, a restless shoal of marine life in chilly blue light.</p>" +
    "<img class=\"img-responsive pull-left col-sm-6 rounded story-pic\" src=\"/images/IMG_20180211_054856.jpg\"><br>"+
    "<p>If you have never participated in a hackathon of any kind, you might have these excuses bouncing around your head in your anxiety:</p>" +
	"<ol>" +
	"<li>I don't have the skills to solve these problems!</li>" +
	"<li>I won't be as good as the more experienced programmers that attend!</li>" +
	"<li>I won't know anyone there!</li>" +
	"<li>I can't possibly write anything good in such a short period of time!</li>" +
	"</ol>" +
	"<p>These are the wrong way to think about hackathons! Instead, tell yourself these:</p>" +
	"<ol>" +
	"<li><strong>Everyone brings something different to the table.</strong> Almost no one signs up because they saw a challenge and said, \"I already know how to fix that.\" Sign up because you have no idea how it's going to happen, but you really WANT to know and want to be part of it! You do have skills, and the problem might be solvable because of that one library you happen to know, or that one class you took ages ago, or that one book you read recently.</li>" +
	"<li><strong>You're there to learn!</strong> It's great if you can solve the challenge of a hackathon easily, but then what would you have gotten out of it?</li>" +
	"<li><strong>You're there to network and make friends!</strong> The best way to get to know people is to work hard on a cool project together. You get to share talents and work towards a goal instead of making small talk, and you can feel awfully close after knowing your whole team knows what you all smell like at 4:30am.</li>" +
	"<li><strong>You're there to make a good start on something that can inspire and expand.</strong> Do NOT expect to finish! If it were possible to finish this polished project easily in one weekend, it wouldn't have been necessary to pull together a hackathon dream team to begin with.</li>" +
	"</ol>" +
    "<p>All that said, it is incredibly satisfying when things come together in some form by the end of the hackathon. We weren't allowed to give live demos, but the sleepless night was so worthwhile: the excitement on Dr. Heiss's face, the thrill of describing our prototype on stage, and the day and week after the hackathon, when the team checked in and said, \"Cool, what next?\"</p>"+
    "<p>Find Team Goop's work at <a href=\"https://github.com/HackTheDeep/teamgoop\">https://github.com/HackTheDeep/teamgoop</a>.</p>"
